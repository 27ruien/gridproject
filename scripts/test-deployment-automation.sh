...
assert_contains scripts/update-dev.sh "FRONTEND_BASE_PATH=\"\\\$FRONTEND_BASE_PATH\"" "Dev update must pass the frontend base path to deploy-dev.sh"
assert_contains scripts/update-dev.sh "FRONTEND_API_BASE_PATH=\"\\\$FRONTEND_API_BASE_PATH\"" "Dev update must pass the frontend API base path to deploy-dev.sh"
assert_contains scripts/update-dev.sh "FRONTEND_DEPLOY_DIR=\"\\\$FRONTEND_DEPLOY_DIR\"" "Dev update must pass the frontend deploy directory to deploy-dev.sh"
assert_contains scripts/update-dev.sh "PUBLIC_HOST=\"\\\$PUBLIC_HOST\"" "Dev update must pass the public host to deploy-dev.sh"
...
assert_contains scripts/update-dev.sh 'install -d.*FRONTEND_DEPLOY_DIR' "Dev update must initialize the frontend deploy directory"
assert_contains scripts/update-dev.sh 'chown -R.*FRONTEND_DEPLOY_DIR' "Dev update must repair ownership of the frontend deploy directory"
assert_contains scripts/update-dev.sh 'safe_remove_frontend_slot.*FRONTEND_PREVIOUS_DIR' "Dev update must safely clean the previous frontend slot"
assert_contains scripts/update-dev.sh 'safe_remove_frontend_slot.*FRONTEND_NEXT_DIR' "Dev update must safely clean the next frontend slot"
assert_contains scripts/update-dev.sh "sudo -u \"\\\$DEPLOY_USER\" test -w \"\\\$FRONTEND_DEPLOY_DIR\"" "Deploy user must be able to write to the frontend deploy directory"
assert_not_contains scripts/update-dev.sh 'chmod[[:space:]]+777' "Dev update must not use world-writable permissions"
assert_contains scripts/update-dev.sh 'http://127\.0\.0\.1:3000/api/health' "Dev update must check the API on port 3000"
assert_contains scripts/update-dev.sh '/tool/dev/project/' "Dev update must check the configured Dev frontend path"
assert_contains scripts/update-dev.sh "Host: \\\$\\{PUBLIC_HOST\\}" "Frontend health check must use the configured public Host header"
...
assert_contains scripts/deploy-dev.sh 'FRONTEND_BASE_PATH' "Frontend base path must be configurable"
assert_contains scripts/deploy-dev.sh 'FRONTEND_API_BASE_PATH' "Frontend API base path must be configurable"
assert_contains scripts/deploy-dev.sh 'FRONTEND_DEPLOY_DIR' "Frontend deploy directory must be configurable"
assert_contains scripts/deploy-dev.sh 'PUBLIC_HOST' "Public host must be configurable"
assert_contains scripts/deploy-dev.sh 'VITE_API_BASE_PATH' "Frontend build must use the API base path variable"
assert_contains scripts/deploy-dev.sh "Host: \\\$\\{PUBLIC_HOST\\}" "Nginx API health check must use the configured public Host header"
