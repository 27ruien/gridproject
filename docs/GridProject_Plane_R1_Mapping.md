...
| Project Header | The project name competed with the app-level `Project Space` title, and the first R1 pass still exposed status as a permanent form control. | Project identity is the visual anchor. Name, status, lead, and actions stay near the title; overview copy follows. Detailed properties are available in context or overview instead of a full-width strip. | Remove the app-level project title. Lead with a compact project mark, 20px name, status lozenge, owner avatar, edit, and overflow. Keep one-line description. Move owner/team/key dates/health/progress/open/risk into a keyboard-accessible project-properties popover; expose only real risk inline. |
...
| Toolbar | The baseline expanded every field, while the first R1 mobile pass still wrapped view, search, filter, sort, display, and create across three rows. | View context, search, filters, display controls, and create action share one compact row. Active constraints become removable chips. | Keep view, search, filter, and `New item` on the mobile first row. Merge sorting and density into the View Options popover. Reuse existing URL-backed values and render a second row only when filter chips exist. |
...
## R1 Visual Acceptance Revision

- Project status is presented as a lozenge and opens an editing menu only on demand.
- Health, progress, dates, teams, and other operational properties are not peer-level header metrics.
- The issue list is a continuous work surface with dividers, not a rounded table card.
- Dense list and board screenshots use local-only QA scenarios and never write to an API or database.
- Sidebar project context is presented as an A/B screenshot decision; no navigation variant is declared accepted in this revision.
